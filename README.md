...
pip install -U pip
pip install -e .[dev]
# or use poetry / pipx if project uses it
```

Run any database migrations or data bootstrap steps if the repo documents them (check `src/quant_research_starter/api/README.md`). -->

<!-- ### 2) Start the Python backend (dev server) : to be updated soon

If the backend is FastAPI / Flask, typical commands:
...
Confirm the REST endpoints (e.g., `GET /health`) are reachable at `http://localhost:8000`. -->
...
```
<!-- 
* The dev server typically runs at `http://localhost:3000` (or the port shown on start).
* The frontend dev server is configured to proxy `/api` to `http://localhost:8000` — see `vite.config.js` or `webpack.dev.js` for proxy settings. -->

<!-- If environment variables are required, copy `.env.example` → `.env.local` and edit values. -->
